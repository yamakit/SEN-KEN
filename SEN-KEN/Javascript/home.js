      ...
      label: function (tooltipItems, data) {
        ...
        for (g = 0; g < assumed * 5; g++) {
          if (tooltipItems.datasetIndex == graphArray[g]["datasetIndex"] && tooltipItems.index == graphArray[g]["index"]) {
            ...
            console.log(chip, "正解数");
            console.log(dale, "解いた数");
          }
        }
        return ["名前：" + data.datasets[tooltipItems.datasetIndex].label, "問題を解いた数：" + dale + "問", "正解した数：" + chip + "問", "正解率：" + Math.round(tooltipItems.yLabel) + "%"];
      },
    },
  },
  title: {
...
var graphArray = [];
function send() {
  //データベースから直近◯日分のデータの数だけデータを取得、グラフを描画、表に数字を表示
  ...
  $.ajax({
    ...
    .done(function (data) {
      ...
      if (button1.length == 0) {
        ...
        percentage1 = Math.round((button1judge.length / button1.length) * 100);
      }
      if (button2.length == 0) {
        percentage2 = 0;
      } else {
        percentage2 = Math.round((button2judge.length / button2.length) * 100);
      }
      if (button3.length == 0) {
        percentage3 = 0;
      } else {
        percentage3 = Math.round((button3judge.length / button3.length) * 100);
      }
      if (button4.length == 0) {
        percentage4 = 0;
      } else {
        percentage4 = Math.round((button4judge.length / button4.length) * 100);
      }
      if (button5.length == 0) {
        percentage5 = 0;
      } else {
        percentage5 = Math.round((button5judge.length / button5.length) * 100);
      }
      if (button6.length == 0) {
        percentage6 = 0;
      } else {
        percentage6 = Math.round((button6judge.length / button6.length) * 100);
      }
      if (button7.length == 0) {
        percentage7 = 0;
      } else {
        percentage7 = Math.round((button7judge.length / button7.length) * 100);
      }
      if (button8.length == 0) {
        percentage8 = 0;
      } else {
        percentage8 = Math.round((button8judge.length / button8.length) * 100);
      }
      if (button9.length == 0) {
        percentage9 = 0;
      } else {
        percentage9 = Math.round((button9judge.length / button9.length) * 100);
      }
